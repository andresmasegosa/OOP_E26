...
All exercises work on [`E_ChessGame.java`](E_ChessGame.java). Exercises 1, 3 and 4 are
the core; do them in order. Exercise 2 is a stretch goal.
...
Run files `A_` to `E_`. Then, in `E_ChessGame`'s `main`:
...
- Implement it. Hint: you will probably need a second 8x8 array (`int[8][8]`), a
  *parallel array* that `movePiece` must keep in sync with the board.
...
In `main` there are two commented-out lines that conjure a second white queen out of
thin air. Uncomment them and run.
